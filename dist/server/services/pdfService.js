...
exports.generateLeveragedBuybackPdf = generateLeveragedBuybackPdf;
exports.generateValuationReportPdf = generateValuationReportPdf;
exports.generateRentalTaxStatementPdf = generateRentalTaxStatementPdf;
...
function renderLeveragedBuybackHtml({ scenario, notes }) {
    ...
    const outcomeDescription = metrics.netGain > 0
    ...
    const formatPercentValue = (value) => `${percentFormatter.format(value)} %`;
    const subtitleParts = [
...
function renderRentalTaxStatementHtml({ formType, taxYear, propertyName, propertyAddress, payload, computed, notes, generatedAt }) {
    ...
    const formatCurrencyOrDash = (value) => typeof value === 'number' && Number.isFinite(value) ? formatCurrency(value) : '—';
    const formatRateOrDash = (value) => typeof value === 'number' && Number.isFinite(value) ? `${value.toFixed(2)} %` : '—';
...
function renderValuationReportHtml(snapshot) {
...
async function renderPdfFromHtml(html) {
    const chromiumArgs = new Set([...chromium_1.default.args]);
    ...
    if (!executablePath) {
        const error = new Error("Chromium n'est pas disponible dans l'environnement d'exécution. Fournissez PUPPETEER_EXECUTABLE_PATH ou vérifiez votre dépendance @sparticuz/chromium.");
        // @ts-expect-error ajout d'un status HTTP personnalisé
        error.status = 500;
        throw error;
    }
    const headlessMode = isWindows && chromium_1.default.headless === 'shell' ? 'new' : chromium_1.default.headless ?? 'new';
    const launchOptions = {
...
async function generateLeveragedBuybackPdf(payload) {
    return renderPdfFromHtml(renderLeveragedBuybackHtml(payload));
}
async function generateValuationReportPdf(snapshot) {
    return renderPdfFromHtml(renderValuationReportHtml(snapshot));
}
async function generateRentalTaxStatementPdf(payload) {
