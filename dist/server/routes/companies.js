...
const optionalTrimmedString = zod_1.z
    ...
    .optional();
const optionalLongText = zod_1.z
    .preprocess((value) => {
    if (value === undefined || value === null) {
        return undefined;
    }
    ...
    .optional();
const optionalNullableTrimmedString = zod_1.z
    .preprocess((value) => {
    if (value === undefined) {
        return undefined;
    }
    ...
    return value;
}, zod_1.z.string().max(255).nullable())
    .optional();
const optionalNullableLongText = zod_1.z
    .preprocess((value) => {
    if (value === undefined) {
        return undefined;
    }
    ...
    return value;
}, zod_1.z.string().max(5000).nullable())
    .optional();
const decimalNumber = zod_1.z.preprocess((value) => {
...
function serializeShareholderLink(link) {
    return {
        id: link.id,
        role: link.role,
...
function serializeShareClass(shareClass) {
    return {
        id: shareClass.id,
...
function serializeShareTransaction(transaction) {
    return {
        id: transaction.id,
...
const optionalDecimal = zod_1.z
    ...
    .optional();
const optionalPercentage = zod_1.z
    .preprocess((value) => {
    if (value === '' || value === null || value === undefined) {
        return undefined;
    }
...
const optionalBoolean = zod_1.z
    ...
    .optional();
const shareholderTypeInput = zod_1.z
    .string()
    .trim()
    .min(1)
    .transform((value) => value.toUpperCase())
    .pipe(zod_1.z.enum(['PERSON', 'CORPORATION']))
    .optional();
const shareholderInputSchema = zod_1.z.object({
...
const shareholderUpdateSchema = zod_1.z
    .object({
...
const statementLineInputSchema = zod_1.z.object({
    category: zod_1.z.string().trim().min(1),
    label: zod_1.z.string().trim().min(1),
    amount: decimalNumber,
    orderIndex: zod_1.z.coerce.number().int().min(0).optional(),
    metadata: optionalLongText
});
const corporateStatementBodySchema = zod_1.z.object({
    statementType: zod_1.z.string().trim().min(1),
    ...
    metadata: optionalLongText,
    lines: zod_1.z.array(statementLineInputSchema).optional()
});
const corporateStatementUpdateSchema = zod_1.z
    .object({
...
const corporateResolutionBodySchema = zod_1.z.object({
...
const corporateResolutionUpdateSchema = zod_1.z
    .object({
...
const shareholderLinkBodySchema = zod_1.z
    .object({
    shareholderId: zod_1.z.coerce.number().int().positive().optional(),
...
const shareholderLinkUpdateSchema = zod_1.z.object({
    role: optionalTrimmedString,
    votingPercent: optionalPercentage,
    shareholder: shareholderUpdateSchema.optional()
});
const companyShareholderParamsSchema = zod_1.z.object({
    id: zod_1.z.coerce.number().int().positive(),
    linkId: zod_1.z.coerce.number().int().positive()
});
const shareClassBodySchema = zod_1.z.object({
    code: zod_1.z.string().trim().min(1).max(25),
    description: optionalLongText,
    hasVotingRights: optionalBoolean,
    participatesInGrowth: optionalBoolean,
    dividendPolicy: optionalLongText
});
const shareClassParamsSchema = zod_1.z.object({
    id: zod_1.z.coerce.number().int().positive(),
    shareClassId: zod_1.z.coerce.number().int().positive()
});
const shareTransactionBodySchema = zod_1.z.object({
...
const shareTransactionParamsSchema = zod_1.z.object({
    id: zod_1.z.coerce.number().int().positive(),
    transactionId: zod_1.z.coerce.number().int().positive()
});
const companyStatementParamsSchema = zod_1.z.object({
    id: zod_1.z.coerce.number().int().positive(),
    statementId: zod_1.z.coerce.number().int().positive()
});
const companyResolutionParamsSchema = zod_1.z.object({
    id: zod_1.z.coerce.number().int().positive(),
    resolutionId: zod_1.z.coerce.number().int().positive()
});
...
function serializeCompanyDetail(company) {
    return {
        id: company.id,
        name: company.name,
        province: company.province,
...
companiesRouter.use(authenticated_1.authenticated);
...
companiesRouter.post('/', (0, requireRole_1.requireRole)('ADMIN'), async (req, res, next) => {
    try {
...
companiesRouter.put('/:id', async (req, res, next) => {
    try {
        const { id } = idParamSchema.parse(req.params);
        const data = companyBodySchema.parse(req.body);
        const existing = await prisma_1.prisma.company.findFirst({ where: { id, userId: req.userId } });
        if (!existing) {
            return res.status(404).json({ error: 'Entreprise introuvable.' });
        }
...
companiesRouter.get('/:id/shareholders', async (req, res, next) => {
    try {
        const { id } = idParamSchema.parse(req.params);
        const company = await prisma_1.prisma.company.findFirst({ where: { id, userId: req.userId } });
        if (!company) {
            return res.status(404).json({ error: 'Entreprise introuvable.' });
        }
        const links = await prisma_1.prisma.companyShareholder.findMany({
            where: { companyId: id },
...
companiesRouter.post('/:id/shareholders', async (req, res, next) => {
    try {
        const { id } = idParamSchema.parse(req.params);
        ...
        if (!shareholderId && data.shareholder) {
            const createdShareholder = await prisma_1.prisma.shareholder.create({
                data: {
                    userId: req.userId,
        ...
        const shareholder = await prisma_1.prisma.shareholder.findFirst({
            where: { id: shareholderId, userId: req.userId }
        });
        if (!shareholder) {
            return res.status(404).json({ error: 'Actionnaire introuvable.' });
        }
        const link = await prisma_1.prisma.companyShareholder.create({
            data: {
                companyId: id,
                shareholderId,
                role: data.role ?? null,
...
companiesRouter.put('/:id/shareholders/:linkId', async (req, res, next) => {
    try {
        ...
        const link = await prisma_1.prisma.companyShareholder.update({
            where: { id: linkId },
            data: {
                role: data.role ?? null,
                votingPercent: data.votingPercent ?? null
...
companiesRouter.delete('/:id/shareholders/:linkId', async (req, res, next) => {
    try {
        const { id, linkId } = companyShareholderParamsSchema.parse(req.params);
        const deleted = await prisma_1.prisma.companyShareholder.deleteMany({
            where: { id: linkId, companyId: id, company: { userId: req.userId } }
        });
        if (deleted.count === 0) {
            return res.status(404).json({ error: 'Lien actionnaire introuvable.' });
...
companiesRouter.get('/:id/share-classes', async (req, res, next) => {
    try {
        const { id } = idParamSchema.parse(req.params);
        const company = await prisma_1.prisma.company.findFirst({ where: { id, userId: req.userId } });
        if (!company) {
        ...
        const shareClasses = await prisma_1.prisma.shareClass.findMany({
            where: { companyId: id },
            orderBy: [{ code: 'asc' }, { id: 'asc' }]
        });
...
companiesRouter.post('/:id/share-classes', async (req, res, next) => {
    try {
        const { id } = idParamSchema.parse(req.params);
...
companiesRouter.put('/:id/share-classes/:shareClassId', async (req, res, next) => {
    try {
        const { id, shareClassId } = shareClassParamsSchema.parse(req.params);
        const data = shareClassBodySchema.parse(req.body);
        const existing = await prisma_1.prisma.shareClass.findFirst({
        ...
        const shareClass = await prisma_1.prisma.shareClass.update({
...
companiesRouter.delete('/:id/share-classes/:shareClassId', async (req, res, next) => {
    try {
        const { id, shareClassId } = shareClassParamsSchema.parse(req.params);
        const deleted = await prisma_1.prisma.shareClass.deleteMany({
            where: { id: shareClassId, companyId: id, company: { userId: req.userId } }
        });
        if (deleted.count === 0) {
            return res.status(404).json({ error: 'Classe d\'actions introuvable.' });
...
companiesRouter.get('/:id/share-transactions', async (req, res, next) => {
    try {
        const { id } = idParamSchema.parse(req.params);
        const company = await prisma_1.prisma.company.findFirst({ where: { id, userId: req.userId } });
        if (!company) {
        ...
        const transactions = await prisma_1.prisma.shareTransaction.findMany({
            where: { companyId: id },
...
companiesRouter.post('/:id/share-transactions', async (req, res, next) => {
    try {
        const { id } = idParamSchema.parse(req.params);
        ...
        const shareClass = await prisma_1.prisma.shareClass.findFirst({
        ...
        const transaction = await prisma_1.prisma.shareTransaction.create({
            data: {
                companyId: id,
...
companiesRouter.put('/:id/share-transactions/:transactionId', async (req, res, next) => {
    try {
        const { id, transactionId } = shareTransactionParamsSchema.parse(req.params);
        const data = shareTransactionBodySchema.parse(req.body);
        const existing = await prisma_1.prisma.shareTransaction.findFirst({
        ...
        const transaction = await prisma_1.prisma.shareTransaction.update({
...
companiesRouter.delete('/:id/share-transactions/:transactionId', async (req, res, next) => {
    try {
        const { id, transactionId } = shareTransactionParamsSchema.parse(req.params);
        const deleted = await prisma_1.prisma.shareTransaction.deleteMany({
            where: { id: transactionId, companyId: id, company: { userId: req.userId } }
        });
        if (deleted.count === 0) {
            return res.status(404).json({ error: 'Transaction introuvable.' });
...
companiesRouter.post('/:id/statements', async (req, res, next) => {
    try {
        const { id } = idParamSchema.parse(req.params);
...
companiesRouter.put('/:id/statements/:statementId', async (req, res, next) => {
    try {
        ...
        const existing = await prisma_1.prisma.corporateStatement.findFirst({
        ...
        const statement = await prisma_1.prisma.corporateStatement.update({
...
companiesRouter.delete('/:id/statements/:statementId', async (req, res, next) => {
    try {
        const { id, statementId } = companyStatementParamsSchema.parse(req.params);
        const deleted = await prisma_1.prisma.corporateStatement.deleteMany({
            where: { id: statementId, companyId: id, company: { userId: req.userId } }
        });
        if (deleted.count === 0) {
            return res.status(404).json({ error: 'État financier introuvable.' });
...
companiesRouter.post('/:id/resolutions', async (req, res, next) => {
    try {
        const { id } = idParamSchema.parse(req.params);
        ...
        const resolution = await prisma_1.prisma.corporateResolution.create({
            data: {
                companyId: id,
                type: data.type.trim().toUpperCase(),
...
companiesRouter.put('/:id/resolutions/:resolutionId', async (req, res, next) => {
    try {
        ...
        const existing = await prisma_1.prisma.corporateResolution.findFirst({
        ...
        const resolution = await prisma_1.prisma.corporateResolution.update({
...
companiesRouter.delete('/:id/resolutions/:resolutionId', async (req, res, next) => {
    try {
        const { id, resolutionId } = companyResolutionParamsSchema.parse(req.params);
        const deleted = await prisma_1.prisma.corporateResolution.deleteMany({
            where: { id: resolutionId, companyId: id, company: { userId: req.userId } }
        });
        if (deleted.count === 0) {
            return res.status(404).json({ error: 'Résolution introuvable.' });
