...
const RECURRING_FREQUENCIES: ReadonlyArray<Exclude<Frequency, 'PONCTUEL'>> = [
  'HEBDOMADAIRE',
  'MENSUEL',
  'TRIMESTRIEL',
  'ANNUEL'
...
export interface ExpenseFiscalItem {
  expenseId: number;
  label: string;
  frequency: Frequency;
  occurrences: number;
...
export interface ExpenseFiscalCategory {
  category: string;
  totalAmount: number;
  items: ExpenseFiscalItem[];
}

export interface ExpenseFiscalProperty {
  propertyId: number;
  propertyName: string;
  totalAmount: number;
  categories: ExpenseFiscalCategory[];
}

export interface ExpenseFiscalReport {
  year: number;
  generatedAt: string;
  totalAmount: number;
  properties: ExpenseFiscalProperty[];
}
...
function countOccurrences(expense: ExpenseWithProperty, year: number): number {
  const frequency = (expense.frequency ?? 'PONCTUEL').toUpperCase() as Frequency;
  ...
  if (!RECURRING_FREQUENCIES.includes(frequency as Exclude<Frequency, 'PONCTUEL'>)) {
    return 0;
  }
...
export async function buildExpensesFiscalReport(
  userId: number,
  year: number
): Promise<ExpenseFiscalReport> {
  const expenses = (await prisma.expense.findMany({
    where: { property: { userId } },
    include: { property: { select: { id: true, name: true } } }
  })) as ExpenseWithProperty[];

  const propertyMap = new Map<number, ExpenseFiscalProperty>();
  ...
  expenses.forEach((expense) => {
    ...
    let categoryEntry = propertyEntry.categories.find(
      (category) => category.category.toLowerCase() === normalizedCategory.toLowerCase()
    );
...
export function expensesFiscalReportToCsv(report: ExpenseFiscalReport): string {
  const headers = [
    'Immeuble',
    'Catégorie',
    'Libellé',
