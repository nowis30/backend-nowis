...
export async function saveUserDocumentFile(options: {
  buffer: Buffer;
  userId: number;
  originalName: string;
}): Promise<{ storagePath: string; checksum: string; filename: string }>
{
  const { buffer, userId, originalName } = options;
  const root = getUploadRoot();
  const folder = path.join(root, `user-${userId}`, 'docs');
...
export async function deleteUserDocumentFile(storagePath: string) {
  try {
    await fs.unlink(resolveUserDocumentPath(storagePath));
  } catch (error) {
...
export function resolveUserDocumentPath(storagePath: string): string {
