...
export interface MortgagePeriodStats {
  payment: number;
  interest: number;
  principal: number;
...
export interface AmortizationAnnualAggregate {
  year: number;
...
export interface AmortizationSummary {
  ...
  annualBreakdown: AmortizationAnnualAggregate[];
...
function calculateTotalPeriods(amortizationMonths: number, paymentFrequency: number): number {
...
function advancePaymentDate(previous: Date, frequency: number): Date {
  switch (frequency) {
    case 12:
...
function calculateElapsedPeriods(startDate: Date, paymentFrequency: number, amortizationMonths: number): number {
...
export function buildAmortizationSchedule(
  ...
  const termPeriods = termMonths > 0 ? Math.min(
    schedule.length,
    Math.max(0, Math.round((termMonths / 12) * frequency))
  ) : 0;
  ...
  const annualMap = new Map<number, { interest: number; principal: number; balance: number }>();
  ...
  const annualBreakdown: AmortizationAnnualAggregate[] = Array.from(annualMap.entries())
    .sort(([a], [b]) => a - b)
    .map(([year, values]) => ({
      year,
...
export function calculateMortgagePeriod(mortgage: MortgageLike): MortgagePeriodStats {
  ...
  const scheduledPayment = (
    typeof mortgage.paymentAmount !== 'undefined' && toNumber(mortgage.paymentAmount) > 0
  ...
  const paymentsMade = calculateElapsedPeriods(
